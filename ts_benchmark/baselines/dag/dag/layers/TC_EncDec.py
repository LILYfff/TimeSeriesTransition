...
class TemporalCausalityEncoder(nn.Module):
    ...
    def set_hct_fusion(self, hct_fusion):
        self.hct_fusion = hct_fusion

    def forward(self, x, exog_future, use_exog=True, hct_pack=None):
        ...
        enc_x_out, _ = self.encoder_x(
            patch_x,
            exog_attns=causality_attns,
            attn_alpha=attn_alpha.repeat(
                self.series_dim, 1, 1, 1
            ),
        )

        # ------------------------------------------------------------
        # Historical Conditional Transition injection
        # ------------------------------------------------------------
        # Inserted after the endogenous temporal encoder and before x_head.
        # Channel branch remains untouched.
        if self.hct_fusion is not None and hct_pack is not None:
            enc_x_out = self.hct_fusion(
                current_tokens=enc_x_out,
                hist_src=hct_pack["hist_src"],
                hist_fut=hct_pack["hist_fut"],
                hist_valid=hct_pack["hist_valid"],
